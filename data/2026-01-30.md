...
### [1] [Initial observations in X-point target divertor discharges on MAST-U](https://arxiv.org/abs/2601.21840)
...
TL;DR: MAST-U实验中首次实现了功率超过3MW的H-mode运行，采用双零X点靶（XPT）偏滤器构型，该构型结合了大打击点半径（类似Super-X偏滤器）和额外X点，展现出优于Super-X偏滤器的排气性能。


<details>
  <summary>Details</summary>
Motivation: 研究如何结合多种替代偏滤器构型策略，以改善动量、功率和粒子损失，应对未来聚变反应堆面临的严峻排气挑战。

Method: 在MAST-U装置上进行首次高功率（>3MW）H-mode实验，采用双零X点靶（XPT）偏滤器构型。该构型结合了大打击点半径（类似Super-X偏滤器）和在偏滤器室内靠近分界面的额外X点。

Result: XPT构型展现出优于Super-X偏滤器的排气效益：在二次X点附近更宽的电子密度分布导致更强的等离子体-中性粒子相互作用，表现为更宽的氢发射谱线分布，产生更大的功率和离子汇；更低的靶电子温度和热通量；初步证据显示在瞬态过程中改善了ELM缓冲能力。

Conclusion: XPT构型成功结合了多种偏滤器策略的优势，展示了通过构型组合改善排气性能的潜力，为应对未来反应堆的排气挑战提供了有前景的解决方案。
...
### [2] [Semi-implicit Lax-Wendroff kinetic scheme for hydrodynamic phonon transport](https://arxiv.org/abs/2601.21161)
*Shijie Li,Hong Liang,Songze Chen,Chuang Zhang*

Main category: physics.comp-ph

TL;DR: 提出了一种基于双弛豫时间近似Boltzmann输运方程的半隐式Lax-Wendroff动力学格式，用于固体材料中的声子输运模拟，能够处理不同散射速率下的多尺度热传导现象。


<details>
  <summary>Details</summary>
Motivation: 传统方法在处理声子输运时，由于需要同时考虑正常散射和电阻散射过程，且数值模拟中时间步长和网格尺寸受到声子平均自由程和弛豫时间的限制，难以高效模拟多尺度热传导现象。

Method: 基于双弛豫时间近似的Boltzmann输运方程，采用半隐式Lax-Wendroff动力学格式。时间积分中，散射项采用梯形法则，迁移项采用中点法则。通过重新求解动力学方程来重构界面通量，实现声子迁移和散射在数值时间步内的耦合。使用二阶迎风或中心格式重构界面分布函数及其空间梯度。

Result: 该方法允许网格尺寸和时间步长大于声子平均自由程和弛豫时间（在小Knudsen数极限下）。数值测试表明，该方法能够准确捕捉不同正常或电阻散射速率下的多尺度热传导现象。

Conclusion: 提出的半隐式Lax-Wendroff动力学格式有效解决了声子输运模拟中的多尺度问题，突破了传统方法在网格尺寸和时间步长上的限制，为固体材料热传导的高效数值模拟提供了新途径。
...
### [3] [Acquiring Human-Like Mechanics Intuition from Scarce Observations via Deep Reinforcement Learning](https://arxiv.org/abs/2601.21881)
*Jingruo Peng,Shuze Zhu*

Main category: physics.comp-ph

TL;DR: 提出一种通过跨任务参数切换的强化学习框架，仅需少量观测即可获得鲁棒的力学直觉，并能泛化到远超训练数据的参数范围


<details>
  <summary>Details</summary>
Motivation: 人类仅需少量观测就能准确推断力学结果，这种数据高效学习的能力机制尚不明确，需要建立人工力学直觉的理论框架

Method: 提出强化学习框架：智能体将连续物理观测参数编码为状态，通过跨相邻观测的周期性切换进行训练，仅需2-3个观测即可学习

Result: 智能体获得了鲁棒的力学直觉，能在远超训练数据的参数范围内准确泛化，在速降线和弹性大变形板问题上得到验证

Conclusion: 周期性切换是实现人工力学直觉的原则性途径，当学习到的价值函数在相邻任务参数间保持贝尔曼一致性时，会产生平滑策略捕获低维解流形
...
### [4] [Trapping and cooling mechanisms in blue-detuned magneto-optical traps of molecules](https://arxiv.org/abs/2601.21097)
*Qinshu Lyu,M. R. Tarbutt*

Main category: physics.atom-ph

TL;DR: 该论文分析了蓝失谐分子磁光阱中的冷却机制，重点研究了Zeeman诱导暗态和移动光晶格产生的力，这些机制共同作用将分子输运到MOT中心并进行冷却。


<details>
  <summary>Details</summary>
Motivation: 红失谐分子磁光阱中存在亚多普勒加热与多普勒冷却竞争，导致高温低密度问题。蓝失谐MOT中亚多普勒冷却占主导，云团被压缩，但需要深入理解其冷却机制。

Method: 首先识别蓝失谐MOT中的俘获力，分析Zeeman诱导暗态产生的恢复力；研究由紧密间隔频率分量产生的移动光晶格作用；结合这两种机制分析分子向MOT中心的输运；最后将分析扩展到具有基态超精细结构的分子。

Result: 发现Zeeman诱导暗态通过光子散射率不平衡产生恢复力；移动光晶格产生速度相关力将分子驱动到光晶格速度；在相关磁场范围内，这两种力结合将分子输运到MOT中心；灰光蜜冷却在光场时变偏振驱动的非绝热跃迁辅助下将分子冷却至零速度。

Conclusion: 蓝失谐分子MOT的冷却机制由Zeeman诱导暗态力和移动光晶格力共同实现，这些机制在简单能级结构和具有基态超精细结构的分子中都有效，为分子冷却提供了新见解。
...
### [5] [Applicability of the Dirac-Fock method combined with Core Polarization in calculations of alkali atoms](https://arxiv.org/abs/2601.21723)
*A. A. Bobylev,J. J. Lopez-Rodriguez,P. A. Kvasov,M. A. Reiter,D. A. Solovyev,T. A. Zalialiutdinov*

Main category: physics.atom-ph

TL;DR: 该研究应用核心极化修正的Dirac-Fock方法，在局部Dirac-Hartree-Fock势框架下，精确计算静态标量和张量电偶极极化率，并评估黑体辐射诱导的斯塔克位移和碱金属原子的Bethe对数。


<details>
  <summary>Details</summary>
Motivation: 开发一种精确计算原子静态标量和张量电偶极极化率的方法，用于评估黑体辐射诱导的斯塔克位移，这对于精密原子光谱和量子计量学应用至关重要。

Method: 采用核心极化修正的Dirac-Fock方法，在局部Dirac-Hartree-Fock势框架下进行计算。该方法不仅用于计算黑体辐射诱导的斯塔克位移，还用于计算碱金属原子的Bethe对数。

Result: 获得了理论的黑体辐射诱导斯塔克位移值，并与当代文献数据进行了批判性比较。讨论了该方法的优势和局限性。

Conclusion: 核心极化修正的Dirac-Fock方法在局部Dirac-Hartree-Fock势框架下，能够有效计算原子极化率和相关物理量，但方法存在一定局限性，需要与现有数据对比验证。
...
### [6] [Statistics and systematics of electron EDM searches with BaF](https://arxiv.org/abs/2601.21781)
...
TL;DR: NL-eEDM实验通过钡氟化物分子束测量电子电偶极矩，目前灵敏度受限于统计量，得到d_e=2(3)×10^{-25} e·cm，并提取了可能引起虚假eEDM的系统误差参数限制。


<details>
  <summary>Details</summary>
Motivation: 测量电子电偶极矩(eEDM)是检验标准模型之外物理的重要探针，特别是寻找时间反演对称性破缺的证据。BaF分子因其有利的分子结构特性成为测量eEDM的理想候选体系。

Method: 使用超音速膨胀源产生BaF分子束，采用自旋进动方法测量eEDM。该方法不仅能提取eEDM值，还能提取可能导致虚假eEDM的系统误差参数值。实验通过分析34小时内收集的数据进行统计限制分析。

Result: 目前灵敏度受统计量限制，得到d_e=2(3)×10^{-25} e·cm。从同一数据集中提取了足够强的参数限制，这些参数可能诱导虚假eEDM，主要包括实验基准体积内的电场E和激光场强度。

Conclusion: 总结了实验升级所需步骤以达到竞争性eEDM测量水平，包括使用低温缓冲气体源产生强激光冷却束流和提高荧光收集效率等改进措施。
...
### [7] [Spectral properties of high-order harmonic radiation enhanced by XUV-driven electron-hole dynamics](https://arxiv.org/abs/2601.21825)
*R. Esteban Goetz,Anh-Thu Le*

Main category: physics.atom-ph

TL;DR: 该研究分析了超越标准高次谐波截止能量的高次谐波辐射光谱特性，这种扩展源于XUV+IR组合配置中红外驱动价电子与阳离子空穴的复合过程。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索超越常规高次谐波截止能量的辐射特性，特别是通过XUV+IR组合配置实现的内壳层电子激发与红外驱动复合过程产生的扩展谐波。

Method: 采用XUV+IR组合配置，其中XUV脉冲激发内壳层电子到瞬态价带空穴，红外脉冲驱动价电子与阳离子空穴复合，分析扩展谐波频率处的微观偶极相位对IR-XUV延迟和IR强度的敏感性。

Result: 发现扩展谐波频率处的微观偶极相位对相对IR-XUV延迟和IR强度敏感，而对于啁啾XUV脉冲（时间相干性差），相应信号强度显著下降。宏观谐波辐射中，偶极发射体间的退相干可能导致进一步信号抑制。

Conclusion: 扩展谐波辐射的光谱特性对实验参数高度敏感，特别是IR-XUV延迟、IR强度以及XUV脉冲的时间相干性，这些因素通过影响微观偶极相位和宏观退相干效应共同决定了最终可观测的信号强度。
...
### [8] [Coherent vibrational dynamics in molecular bond breaking: methyl radical umbrella mode probed by femtosecond x-ray spectroscopy](https://arxiv.org/abs/2601.21949)
...
TL;DR: 利用X射线光谱观测甲基自由基在光解离后产生的相干分子振动，通过量子力学模型重建了真实空间振动轨迹


<details>
  <summary>Details</summary>
Motivation: 研究分子键断裂后引发的相干分子振动动力学，特别是甲基自由基在光解离过程中产生的高度激发振动模式

Method: 使用267nm光解离甲基碘产生甲基自由基，通过测量C1s到单占据价轨道的X射线跃迁能量的超快时间变化来观测振动动力学，构建完全量子力学模型表征相干叠加态

Result: 观测到甲基自由基ν2"伞形"振动模式的高度相干激发，振动运动主要通过ν2级数的差频出现在X射线能量位移中，重建的轨迹显示由高度相干激发和强负非谐性主导的显著量子拍频

Conclusion: 成功观测并表征了分子键断裂引发的相干分子振动，通过量子力学模型重建了真实空间振动轨迹，揭示了高度相干激发和强负非谐性对振动动力学的主导作用
...
TL;DR: 提出了一种将耦合元件纳入广义耦合模理论的新方法，使用波导与外部射频源/负载通过环和传输线耦合的简化模型，将复杂的积分-微分系统简化为三个独立的常微分方程组，并开发了用于X波段均匀加速段环耦合器匹配的计算机代码。


<details>
  <summary>Details</summary>
Motivation: 传统耦合模理论在处理波导与外部射频源/负载通过环和传输线耦合时，系统方程变得非常复杂（耦合积分-微分方程），需要寻找简化方法以便于实际工程应用。

Method: 采用结构化波导与外部射频源/负载通过环和传输线耦合的简化模型，将耦合积分-微分系统（Barbashin型退化核）简化为三个独立的常微分方程组：两个描述环激励场分布（输入/输出端），一个描述无环时电子束激励场分布。

Result: 基于该解析模型开发了X波段均匀加速段环耦合器匹配的计算机代码，计算结果显示，在不进行额外匹配的情况下，非均匀段的输入反射系数达到8×10⁻³。

Conclusion: 该方法成功简化了复杂耦合系统的分析，将积分-微分方程转化为更易求解的常微分方程组，为加速器耦合器设计提供了有效的计算工具，在工程应用中取得了良好的匹配效果。
...
### [10] [From Beam to Bedside: Reinforcing Domestic Supply of $^{99}$Mo/$^{99m}$Tc using Novel High-Current D+ Cyclotrons for Compact Neutron Generation and $^{99}$Mo Production](https://arxiv.org/abs/2601.20997)
*Jarrett Moon,Daniel Winklehner,Jose Alonso,Claire Huchthausen,David McClain,Janet Conrad*

Main category: physics.acc-ph

TL;DR: 开发用于医院级同位素生产的新型回旋加速器系统，通过高束流产生中子来生产钼-99，替代传统核反应堆供应链


<details>
  <summary>Details</summary>
Motivation: 锝-99m是美国每年1600多万次诊断程序的关键同位素，传统供应链依赖核反应堆生产钼-99，存在多个脆弱环节，容易导致短缺和延误，影响患者治疗

Method: 开发新型回旋加速器家族（源自IsoDAR中微子实验设计），在1.5 MeV/amu能量下运行氘核束，预期连续束流达5 mA，使用薄铍靶产生中子，通过模拟计算中子产额

Result: 模拟预测该系统可产生约10^13中子/秒的中子产额，理论上足以支持钼-99生产，无需使用高浓铀或依赖国外反应堆，低能氘核束减少了活化和安全问题

Conclusion: 该系统有望实现分散化、医院级的同位素生产，从物理原型到生物医学同位素生产平台的转化路径可行，可减少供应链脆弱性
...
### [11] [Photoionization in KTN deflectors by light in the near-infrared imaging window](https://arxiv.org/abs/2601.21029)
*Samuel Stanek,Harishankar Jayakumar,Christopher Warkentin,James Leger,Aaron Kerlin*

Main category: physics.optics

TL;DR: 测量KTN电光偏转器在近红外生物成像窗口（700-1300 nm）的光电离效应，发现电离率随波长增加而显著降低，为近红外成像应用提供关键定量指导。


<details>
  <summary>Details</summary>
Motivation: KTN电光偏转器具有高扫描速度和高分辨能力，但可见光会引发快速光电离，限制了其偏转范围。虽然最近提出将KTN用于近红外生物成像，但缺乏近红外光电离的定量数据，这阻碍了其在深部组织成像和非线性显微镜中的应用优化。

Method: 使用双光束偏振干涉仪测量KTN偏转器中俘获电荷密度随光子通量的变化，覆盖NIR-I和NIR-IIa生物成像窗口（700-1300 nm）。通过分析电荷密度衰减曲线来研究光电离动力学。

Result: 光电离率随波长增加而显著降低。电荷密度衰减曲线呈现多指数行为，无法用单一陷阱模型解释，表明存在多种陷阱物种或显著的再俘获过程。

Conclusion: 这些测量为KTN基电光偏转器在近红外成像应用中提供了关键定量指导，有助于选择工作波长和电荷扫描占空比，优化其在深部组织成像和非线性显微镜中的性能。
...
### [12] [Wavelength-selective nonlinear wavefront control in resonant thin-film lithium niobate metasurfaces](https://arxiv.org/abs/2601.21382)
*Madona Mekhael,Timo Stolt,Helena Weigand,Kiia Arola,Rachel Grange,Patrice Genevet,Mikko J. Huttunen*

Main category: physics.optics

TL;DR: 基于薄膜铌酸锂的超表面实现了光谱选择性二次谐波生成波前整形，通过共振工程相位控制将1100nm高斯泵浦转换为550nm一阶厄米-高斯模式


<details>
  <summary>Details</summary>
Motivation: 现有非线性超表面方法（通常基于几何相位）缺乏波长选择性，导致非线性响应静态不变。需要开发能够实现光谱选择性非线性波前整形的紧凑器件。

Method: 设计薄膜铌酸锂超表面，通过光谱调谐Mie型共振实现两个具有不同相位响应的区域。利用共振工程相位控制实现光谱选择性二次谐波生成波前整形。

Result: 成功将1100nm附近的高斯泵浦转换为550nm的一阶厄米-高斯模式，同时保持泵浦轮廓。实现了同时的频率转换和空间模式整形。

Conclusion: 该方法为非线性全息及相关应用提供了通往超紧凑可调谐组件的途径，展示了共振工程在非线性波前整形中的潜力。
...
### [13] [Hybrid Barium Titanate Waveguide Designs For Efficient Nonlinear Frequency Conversion](https://arxiv.org/abs/2601.21427)
*Trevor G. Vrckovnik,D. Arslan,F. Eilenberger,Sebastian W. Schmitt*

Main category: physics.optics

TL;DR: 提出基于BaTiO₃-TiO₂混合波导的线性-非线性混合结构，通过选择性掺入TiO₂增强非线性模式重叠，仅依赖模态相位匹配，实现2.75倍二次谐波效率提升。


<details>
  <summary>Details</summary>
Motivation: BaTiO₃作为集成光子材料具有强二阶非线性和电光特性，但其非线性波导工程相对不成熟。传统周期性极化技术因BaTiO₃的高矫顽场、强应变夹持效应、多畴结构和复杂开关动力学而不适用，阻碍了高效二阶频率转换器的发展。

Method: 采用线性-非线性混合波导设计，在BaTiO₃脊波导中选择性掺入TiO₂以增强非线性模式重叠，完全依赖模态相位匹配而非周期性极化。通过耦合模理论模拟确定相位匹配几何结构。

Result: 混合设计相比单片BaTiO₃波导实现了2.75倍的归一化二次谐波产生效率提升。均匀、光刻定义的横截面使该方法具有高度可扩展性。

Conclusion: BaTiO₃-TiO₂混合波导为CMOS兼容的高效二阶器件提供了一条实用路径，适用于集成量子光子学应用。
...
### [14] [Optically reconfigurable canalization of exciton-polaritons in a non-hyperbolic perovskite](https://arxiv.org/abs/2601.21443)
*Jiahao Ren,Olha Bahrova,Feng Jin,Hao Zheng,Dmitry Solnyshkov,Cheng-Wei Qiu,Guillaume Malpuech,Rui Su*

Main category: physics.optics

TL;DR: 在双折射CsPbBr3钙钛矿晶体中实现了非线性激子极化激元凝聚体的可重构运河化传输，通过光学泵浦调控实现了从双曲到抛物型等频轮廓的切换，实现了超过20倍的准直增强。


<details>
  <summary>Details</summary>
Motivation: 极化激元流的按需操控对于纳米光子应用和光子电路具有重要意义。极化激元运河化具有固有的准直和无衍射传输特性，无需波导结构，但先前的研究仅限于具有固有双曲响应的晶体表面且在线性区域工作。本研究旨在探索在没有固有双曲响应的双折射晶体中实现非线性极化激元凝聚体的可重构运河化。

Method: 将双折射CsPbBr3钙钛矿晶体嵌入平面微腔中，利用腔的TE-TM分裂与晶体双折射之间的相互作用产生各向异性能带结构。通过非共振泵浦驱动激子极化激元凝聚到平坦的远场等频轮廓上，实现非线性发射放大。通过重新配置光学泵浦光斑尺寸，在双曲、平坦和抛物型等频轮廓之间切换极化激元凝聚体。

Result: 实验实现了非线性激子极化激元凝聚体的运河化流动，与弧形的等频轮廓相比，实现了超过20倍的准直增强。通过调控泵浦光斑尺寸，成功将非线性极化激元凝聚体切换到双曲和抛物型等频轮廓区域，实现了从发散到准直的不同传播行为重构。

Conclusion: 本研究揭示了一种独特的运河化框架，用于塑造非线性激子极化激元凝聚体流，为基于稳定非线性量子互连的全光极化激元逻辑电路开辟了新机遇。
...
### [15] [Measurement of stray light in the LISA instrument](https://arxiv.org/abs/2601.21501)
*Marco Nardello,Amaël Roubeau-Tissot,Michel Lintz*

Main category: physics.optics

TL;DR: 开发了一种测量复杂光学系统内相干杂散光的方法，通过扫描频率激光插入和信号检测，分析干涉条纹来确定杂散光源及其贡献


<details>
  <summary>Details</summary>
Motivation: 复杂光学系统中的杂散光测量是一项复杂任务，需要一种能够在组装设备内部测量相干杂散光的方法，以确定所有杂散光源及其相对贡献

Method: 基于扫描频率激光插入和仪器所有电信号与光信号的检测，通过分析信号频谱中的干涉条纹来确定杂散光路径与标称路径之间的光程差，并结合光学仿真将测量路径与系统内可能路径关联

Result: 在模拟LISA干涉仪性能的测试平台上进行了测量，展示了光学仿真在预测和解释测量结果方面的准确性

Conclusion: 该方法能够有效测量复杂光学系统中的相干杂散光，通过结合实验测量和光学仿真，可以准确识别杂散光源及其路径，为系统性能评估提供可靠手段
...
### [16] [Non-secular polariton leakage and dark-state protection in hybrid plasmonic cavities](https://arxiv.org/abs/2601.21597)
*Marco Vallone*

Main category: physics.optics

TL;DR: 该论文推导了描述等离子体腔开放系统的非旋波主方程，预测了当环境无法分辨极化子分裂时会出现与标准旋波描述的一阶偏差，包括浴诱导相干性和暗极化子稳定化。


<details>
  <summary>Details</summary>
Motivation: 等离子体腔作为纳米技术关键组件时，辐射和吸收损耗对其电动力学行为的影响是一个主要问题。需要发展能够处理开放系统并保留衰减路径间非旋波干涉的理论框架。

Method: 将等离子体腔视为开放系统，推导了时间局域、完全正定的主方程，该方程保留了衰减路径间的非旋波干涉，并在环境能够分辨极化子分裂时简化为标准旋波描述。

Result: 当环境无法分辨极化子分裂时，理论预测了与旋波泄漏动力学的一阶偏差，包括浴诱导相干性和暗极化子的稳定化。提出了基于极化子分裂与储层线宽比值的简单设计准则。

Conclusion: 时间分辨的泄漏测量（如透射、反射或光致发光）可用于观测这些效应。该理论为等离子体腔的设计和应用提供了新的理论框架和实验验证方法。
...
### [17] [Reverse Energy Flows in Two-Dimensional Photonic Crystals and Analogies with Vortex Formation and Analogous Flows in Hydrodynamics](https://arxiv.org/abs/2601.21704)
*Andrey Pryamikov*

Main category: physics.optics

TL;DR: 该论文研究了二维光子晶体中光子带隙形成与反向电磁能流之间的关联，揭示了这些反向能流源于坡印廷矢量的涡旋结构，其模式类似于流体绕过障碍物时的涡旋形成。


<details>
  <summary>Details</summary>
Motivation: 研究动机在于探索二维光子晶体中光子带隙形成的物理机制，特别是理解线性偏振平面波入射到光子晶体平板时产生的反向电磁能流现象，以及这些能流模式与光子带隙形成之间的内在联系。

Method: 通过分析两种类型的二维光子晶体，研究线性偏振平面波入射到光子晶体平板时产生的透射和反射场中的反向能流。重点考察坡印廷矢量的涡旋结构，并将其与流体力学中障碍物周围的涡旋形成进行类比分析。

Result: 研究发现反向电磁能流源于坡印廷矢量的涡旋结构，这些能流模式与流体绕过障碍物时的涡旋形成具有显著相似性。坡印廷矢量涡旋的几何形状和动力学特性决定了入射电磁能量是被阻碍（导致光子带隙形成）还是被引导通过结构（实现透射）。

Conclusion: 光子晶体中光子带隙的形成与坡印廷矢量涡旋结构密切相关。反向电磁能流的出现是光子带隙形成的直接表现，而涡旋的几何和动力学特性决定了光子晶体对电磁能量的传输特性，为理解光子带隙的物理机制提供了新的视角。
...
### [18] [Mode Switching Through Exceptional Points Induced by Lasing-Inversion Coupling](https://arxiv.org/abs/2601.21907)
*Xingwei Gao,Cheng Guo,David Burghoff*

Main category: physics.optics

TL;DR: 该论文研究了光学腔中增益-损耗耦合产生的异常点(EPs)如何影响激光动力学，揭示了第二个阈值源于激光场、反转粒子数和暗腔模式之间的动态耦合，这种耦合产生额外的异常点，控制着单模激光和不同重复频率频率梳之间的切换。


<details>
  <summary>Details</summary>
Motivation: 异常点附近的大模态重叠会加剧增益竞争，有利于单模激光。然而，最近研究发现靠近异常点会出现自调制现象，将激光模式转变为频率梳。这种异常点启发的梳状形成暗示了一个先前未考虑的机制，该机制克服了强增益竞争并使第二个模式达到阈值。本文旨在揭示这一机制。

Method: 使用从第一性原理导出的布洛赫耦合模理论，分析激光场、反转粒子数和暗腔模式之间的动态耦合。研究这些耦合如何产生额外的异常点，并控制激光动力学行为。

Result: 研究发现第二个阈值源于激光场、反转粒子数和暗腔模式之间的动态耦合。激光-反转耦合产生额外的异常点，其光谱结构控制着单模激光和不同重复频率频率梳之间的切换。这种动态耦合机制克服了强增益竞争，使第二个模式能够达到阈值。

Conclusion: 这种阈值以上模式切换机制为可调谐光子系统提供了新的机会，包括自适应光通信链路和双梳光谱学。研究揭示了异常点附近激光动力学的复杂动态耦合机制，为设计新型光子器件提供了理论基础。
...
### [19] [Broadband Second Harmonic Generation using Fixed-Period Periodically Poled Lithium Niobate](https://arxiv.org/abs/2601.21932)
...
<details>
  <summary>Details</summary>
Motivation: 传统PPLN的二次谐波生成带宽受准相位匹配条件限制，而啁啾或非周期PPLN结构需要定制且商业可用性差，需要寻找使用标准PPLN器件的实用宽带SHG方案

Method: 采用包含多个固定极化周期的标准PPLN晶体，通过自由空间中角度调谐晶体相对于输入光束的方向来实现宽带操作，研究不同入射角度下的角度调谐效应

Result: 通过角度调谐实现了1.6倍的二次谐波生成带宽增强，验证了使用标准PPLN器件实现宽带SHG的可行性

Conclusion: 使用标准多周期PPLN晶体结合角度调谐是一种实用高效的宽带二次谐波生成策略，无需定制非周期或啁啾结构
...
### [20] [Assessing engineering wake models against operational data: insights from the Lillgrund wind farm wake steering campaign](https://arxiv.org/abs/2601.21035)
...
TL;DR: 该研究利用Lillgrund海上风电场的SCADA和LiDAR同步测量数据，评估了四种分析型尾流模型组合在真实运行条件下的性能，重点关注尾流速度亏损和功率预测精度。


<details>
  <summary>Details</summary>
Motivation: 在真实运行条件下验证工程尾流模型对于提高风电场性能预测至关重要。现有研究缺乏基于现场测量数据的系统模型评估，特别是在主动尾流转向应用背景下。

Method: 使用Lillgrund海上风电场在Horizon 2020 TotalControl项目中收集的独特数据集，结合同步SCADA和LiDAR测量数据。评估了四种分析型尾流模型组合，采用不同的速度亏损、附加湍流、尾流叠加和偏转公式，在DNV开发的LongSim软件中实现。分析重点为时间平均尾流速度亏损和涡轮机级/风电场级功率输出，使用平均绝对误差指标量化模型精度。

Result: 模型能够再现各种大气条件下的尾流亏损趋势和尾流偏转，归一化速度亏损误差在7%至15%之间。功率预测误差随风电场深度增加而增大，涡轮机级误差在3%至23%之间，风电场级误差在-13%至+30%之间。某些分析模型达到了与报道的LES结果相当的精度，同时计算成本显著降低。

Conclusion: 现场测量活动对于基准测试工程尾流模型具有重要价值，研究结果为尾流转向应用中模型保真度与操作实用性之间的权衡提供了参考。分析模型在保持合理精度的同时大幅降低了计算成本，适合实际工程应用。
...
### [21] [Reviving the Suspension Balance Model](https://arxiv.org/abs/2601.21087)
*Mu Wang,Tingtao Zhou,John F. Brady*

Main category: physics.flu-dyn

TL;DR: 该研究验证了悬浮平衡模型(SBM)的物理基础，通过分析两相流中粒子相应力与悬浮应力的关系，澄清了Nott等人提出的争议，证明SBM的关键假设是正确的。


<details>
  <summary>Details</summary>
Motivation: Nott等人对悬浮平衡模型(SBM)的物理基础提出质疑，认为悬浮应力的流体力学贡献不应出现在粒子相动量平衡中，而应被不同的粒子相应力替代。这种争议需要澄清以确定SBM模型的正确性。

Method: 1. 通过简单的双粒子计算确定正确的力分配方式；2. 从热力学角度验证该分配的合理性，证明其能给出布朗胶体的正确平衡渗透压；3. 使用斯托克斯动力学悬浮流变学定量比较悬浮应力与粒子相应力的流体力学贡献。

Result: 1. 确定了正确的力分配方式，该分配在热力学上一致且能给出正确的平衡渗透压；2. 定量证明悬浮应力的流体力学贡献与粒子相应力几乎完全相同；3. 唯一区别是：当用于预测粒子通量时，必须从悬浮应力中移除孤立单粒子流体力学应力贡献（爱因斯坦粘度修正）。

Conclusion: 研究结果验证了SBM的关键假设，恢复了其物理基础。悬浮平衡模型中使用悬浮应力作为粒子相应力的方法是正确的，只需在应用时移除爱因斯坦粘度修正项。
...
### [22] [Asymmetric velocity boundary conditions lead to zonal flow in centrifugal convection](https://arxiv.org/abs/2601.21396)
*Jun Zhong,Chao Sun*

Main category: physics.flu-dyn

TL;DR: 该研究通过直接数值模拟快速旋转环形离心对流，探究混合速度边界条件和几何曲率对流动组织和传热的影响。研究发现不同边界条件组合导致显著不同的传热效率和流动模式，其中应力自由/应力自由边界传热最强，而应力自由/无滑移边界因强带状流发展导致传热显著抑制。


<details>
  <summary>Details</summary>
Motivation: 研究动机在于理解快速旋转下流动的准二维化特性，以及混合（不对称）速度边界条件和几何曲率如何影响离心对流中的流动组织和传热效率。特别关注大尺度状态形成所需的长时间自旋过程。

Method: 采用二维直接数值模拟方法，研究快速旋转环形离心对流。考虑四种边界条件组合：无滑移/无滑移(INON)、无滑移/应力自由(INOS)、应力自由/无滑移(ISON)和应力自由/应力自由(ISOS)。固定半径比η=0.5，在瑞利数Ra∈[10^6,10^9]范围内进行模拟。通过改变半径比η研究曲率不对称性的影响，并使用自由对流边界层模型分析边界热通量不对称性。

Result: 1. 传热效率排序：ISOS最强，其次是INOS和INON，ISON因强带状流发展而显著抑制传热。2. 大尺度环流主导的三种情况中，努塞尔数Nu遵循接近Nu∼Ra^{0.27}的经典型标度律；带状流分支则显示弱得多的标度律Nu∼Ra^{0.1}，且流动各向异性显著（径向雷诺数Re_r≪方位角雷诺数Re_φ）。3. 耗散分析表明带状流形成伴随着从边界层主导耗散向相对较低且均匀的体耗散转变，这与剪切诱导羽流抑制一致。4. 增加半径比η会减弱曲率不对称性并破坏带状流状态，在平面极限下导致滚流主导的对流。

Conclusion: 混合速度边界条件和几何曲率对快速旋转离心对流的流动组织和传热有重要影响。应力自由/应力自由边界条件产生最强传热，而应力自由/无滑移边界因带状流发展导致传热显著抑制。带状流状态伴随弱传热标度律和强流动各向异性。曲率不对称性在稳定带状流状态中起关键作用，增加半径比会破坏该状态并恢复滚流主导对流。边界热通量不对称性与体温度不对称性相关，可用自由对流边界层模型解释。
...
### [23] [Nonlinear Aerodynamic Response and an Equivalent Static Wind-resistant Design for Anticlastic Conical Tensile Membranes](https://arxiv.org/abs/2601.21777)
*Ajay Kumar,Budhaditya De,Sudib Kumar Mishra,Devasmit Dutta*

Main category: physics.flu-dyn

TL;DR: 研究锥形张拉膜结构在随机风荷载下的空气动力响应，提出基于阵风响应因子和非线性调整因子的等效静力抗风设计方法


<details>
  <summary>Details</summary>
Motivation: 锥形张拉膜结构具有美观、经济、抗雨雪荷载等优点，但其在几何非线性下的复杂空气动力行为过去研究不足，需要深入分析其在随机风荷载下的响应并建立实用的抗风设计方法

Method: 采用大涡模拟生成大气边界层中的随机风荷载，对开敞式（无围护）和封闭式（有围护）锥形张拉膜结构进行非线性时程分析，研究关键参数（空气动力粗糙高度、膜结构矢跨比、膜预张力）的影响，建立基于阵风响应因子和非线性调整因子的等效静力抗风设计方法

Result: 增加预张力和矢跨比可增强膜结构刚度，但预张力影响更显著；增加粗糙高度会增强湍流，导致峰值荷载/响应增大；建立了阵风响应因子和非线性调整因子的系统数据，提出了多元线性回归模型进行预测，并进行了概率分析确定设计值

Conclusion: 提出了锥形张拉膜结构的等效静力抗风设计方法，通过阵风响应因子和非线性调整因子简化设计过程，避免了复杂的非线性动力分析，为工程实践提供了实用设计工具
...
### [24] [Investigation of Wake Dynamics of a Slender Symmetric Trailing Edge Hydrofoil](https://arxiv.org/abs/2601.21939)
*Gabriele Gaiti,Chirag Trivedi,Kristian F. Sagmo*

Main category: physics.flu-dyn

TL;DR: 结合LES模拟与PIV实验研究钝尾缘对称水翼湍流尾迹，通过POD分析揭示主导相干结构，验证数值模拟对振动相关水翼动力学的适用性。


<details>
  <summary>Details</summary>
Motivation: 准确预测水翼尾迹动力学对于减轻涡激振动和提高水力机械性能至关重要。传统湍流模型难以捕捉控制尾迹行为的非定常相干结构，特别是对于高雷诺数下工作的细长水翼。

Method: 结合尺度解析数值模拟（高分辨率LES，约5亿节点）和粒子图像测速（PIV）测量，研究零攻角下对称钝尾缘水翼的湍流尾迹。在雷诺数约7.5×10⁵下分析流动，接近尾迹-结构相互作用起始点。对PIV数据应用本征正交分解（POD）提取主导相干结构并量化其对湍流动能的贡献。

Result: POD分析显示能量分布在多个模态中，主导模态捕捉主要尾迹动力学，高阶模态形成与冯·卡门涡脱落相关的耦合振荡对。PIV-LES一致性表明，中心尾迹测量与数值模拟结合可实现完整尾迹重建，并验证了振动相关水翼动力学的建模。

Conclusion: 通过结合高分辨率LES模拟和PIV实验测量，成功研究了高雷诺数下水翼湍流尾迹的相干结构。该方法能够准确重建完整尾迹场，为预测和控制水翼涡激振动提供了有效的数值模拟验证框架。
...
### [25] [Exact coherent structures as building blocks of turbulence on large domains](https://arxiv.org/abs/2601.21970)
*Dmitriy Zhigunov,Jacob Page*

Main category: physics.flu-dyn

TL;DR: 该研究提出了一种通过空间拼接小计算域精确解来构建扩展湍流系统中新简单不变解的方法，应用于垂直扩展Kolmogorov流，发现了由小域相对周期轨道组合形成的两环面解。
...
Method: 通过空间拼接小计算域精确解构建新不变解，使用基于梯度的优化方法最小化标量损失函数，该函数针对流动空间掩蔽区域的自回归特性。应用于垂直扩展Kolmogorov流，首先识别大量相对周期轨道，然后展示垂直堆叠不同小域RPOs可形成较大域中动态相关的两环面解。

Result: 成功识别了大量由高耗散小域解与层流斑块组合的相对周期轨道。垂直堆叠不同小域RPOs可形成鲁棒的两环面猜测解。优化程序还能识别在子域中局部跟随小域RPO多个周期的"湍流"轨迹。这些组合之所以可能，是因为流动在长时间内处于可有效视为一对弱耦合小域系统的状态，这与高耗散流动结构的屏蔽效应有关。

Conclusion: 通过空间拼接小计算域精确解的方法成功扩展了动力系统方法到空间扩展湍流系统，为理解复杂湍流结构提供了新途径，揭示了高耗散结构的屏蔽效应如何使弱耦合小域系统近似成为可能。
...
### [26] [Learning Transient Convective Heat Transfer with Geometry Aware World Models](https://arxiv.org/abs/2601.22086)
*Onur T. Doganay,Alexander Klawonn,Martin Eigel,Hanno Gottschalk*

Main category: physics.flu-dyn

TL;DR: 提出一种基于几何感知的世界模型架构，用于学习瞬态物理过程，通过双重条件机制和任意通道维度支持，在2D瞬态CFD问题上验证了复杂时空动态的再现能力。


<details>
  <summary>Details</summary>
Motivation: PDE模拟在工程和物理中至关重要，但实时应用计算成本高昂。现有生成式AI视频架构缺乏物理模拟所需的特定控制和数据兼容性，需要专门架构来学习瞬态物理过程。

Method: 提出几何感知世界模型架构，源自LongVideoGAN，包含：(1)双重条件机制（全局物理参数+局部几何掩码），(2)支持任意通道维度的架构适应，超越标准RGB限制。在2D瞬态CFD问题（浮力驱动流对流传热耦合固体热流）上评估。

Result: 条件模型成功再现训练数据的复杂时间动态和空间相关性。在未见几何配置上评估泛化能力，展示了受控模拟合成的潜力，但也揭示了空间精度方面的局限性。

Conclusion: 几何感知世界模型架构能够学习瞬态物理过程，为PDE模拟提供有前景的替代方案，但在处理分布外样本时空间精度仍需改进。
