...
TL;DR: 本文报道了在单个囚禁40Ca⁺离子的3²D₅/₂和4²S₁/₂态之间朗德g因子比值的精密测量，分别使用低温表面电极射频Paul阱和室温永磁Penning阱两种装置，测量精度相比此前工作提升40倍以上。


<details>
  <summary>Details</summary>
Motivation: 实现对朗德g因子比值的高精度测量，以检验量子电动力学理论、确定精细结构常数，或为量子信息处理和精密测量提供更精确的原子参数。

Method: 在两种不同离子阱装置（低温表面电极射频Paul阱和室温永磁Penning阱）中囚禁单个40Ca⁺离子，分别测量其3²D₅/₂和4²S₁/₂态的朗德g因子比值，并评估系统误差。

Result: Penning阱测量得到比值为0.599 488 813 3(2)，相比此前工作不确定度降低40倍以上；射频Paul阱测量得到一致值为0.599 488 813(6)；两系统的系统偏移均远低于各自的统计不确定度。

Conclusion: 两种不同离子阱装置测得的结果高度一致，且系统误差可控，验证了测量结果的可靠性，为相关精密测量物理研究提供了更高精度的g因子比值数据。
...
TL;DR: 本文利用从头计算方法研究了Cu K壳层跃迁在电离阈值附近的卫星强度演化，模拟了Kα₁和Kα₂光谱，发现低于阈值的卫星强度来源于Cu(I)和Cu(II)氧化物相中的共振1s→3d和1s→4p激发。


<details>
  <summary>Details</summary>
Motivation: 研究铜K壳层跃迁在电离阈值附近卫星强度的演化机制，通过理论方法理解低于阈值卫星强度的物理起源。

Method: 采用标准先进的从头计算方法（ab initio methods），计算所有Cu K壳层跃迁，模拟完整Kα₁和Kα₂光谱，所有跃迁参数及shake概率均由理论确定。

Result: 标准从头计算方法与实验吻合良好，能较好误差范围内模拟电离阈值附近的强度演化；低于阈值的卫星强度来源于Cu(I)和Cu(II)氧化物相中的共振1s→3d和1s→4p激发。

Conclusion: 标准从头计算方法可用于准确模拟Cu K壳层跃迁光谱及卫星强度演化，揭示了低于阈值卫星强度源于特定共振激发过程。
...
TL;DR: 本文研究了O₂分子在XFEL短脉冲照射下的行为，发现从O⁺+O⁺通道提取平衡核间距比从更高电荷通道更准确，挑战了传统库仑爆炸成像的理解。


<details>
  <summary>Details</summary>
Motivation: 探究XFEL脉冲与O₂分子的相互作用机制，特别是不同碎裂通道中动能释放（KER）与分子平衡核间距的关系，检验传统库仑爆炸成像理论的适用性。

Method: 使用570 eV光子能量照射O₂分子，计算不同碎裂通道（O⁺+O⁺、O²⁺+O²⁺等）的原子碎片动能释放总和（KER），并基于势能曲线进行分析比较。

Result: O⁺+O⁺通道比更高电荷通道（如O²⁺+O²⁺）更适合提取平衡核间距；但O⁺+O⁺通道的KER与仅基于库仑排斥的计算结果不一致。

Conclusion: 传统库仑爆炸成像中认为更高电荷通道更优的准经典观点需要修正，分子碎裂动力学比简单库仑排斥模型更复杂。
...
TL;DR: 本文提出一种普适性方案，通过光致电离激光冷却原子在真空中产生等离子体源，有效消除里德伯原子光镊阵列中不受控的杂散电场，实现了从斯塔克电离连续谱到稳定相干单里德伯态激发的恢复。


<details>
  <summary>Details</summary>
Motivation: 里德伯原子对外部电场极度敏感，杂散电场会严重破坏其量子控制，而现有方法难以高效、普适地消除此类电场。

Method: 利用光致电离激光冷却原子在真空中原位产生等离子体源，通过等离子体中和效应消除杂散电场。

Result: 初始存在大未知杂散电场导致的斯塔克电离连续谱，经该方法完全消除电场后，恢复了稳定的单个里德伯态相干激发。

Conclusion: 该方案可直接应用于现有里德伯原子平台，并对其他对杂散电场敏感的实验中具有潜在用途。
...
TL;DR: 本文通过傅里叶成像测量了冷87Rb原子云超辐射发射的空间模式，观察到高度定向的环状发射结构，对应单个集体跳跃算符。通过空间滤波隔离该通道，发现超辐射爆发的强度与原子数呈超线性标度，实验结果与理论模型吻合良好，证明了集体跳跃算符可被测量和操控。


<details>
  <summary>Details</summary>
Motivation: 测量和验证超辐射发射中集体跳跃算符的空间模式，证明其可观测性和可操控性，填补超辐射理论中集体算符的实验验证空白。

Method: 使用傅里叶成像测量冷87Rb原子云的超辐射发射空间模式；通过空间滤波隔离特定的集体发射通道；测量强度与原子数的标度关系；与两种理论模型进行对比。

Result: 观察到高度定向的环状发射结构，对应最超辐射模式的单个集体跳跃算符；隔离该通道后观察到典型的超辐射爆发现象，强度随原子数呈超线性标度；实验结果与理论模型吻合良好。

Conclusion: Carmichael等人引入的集体跳跃算符不仅可以在理论上描述超辐射发射，还可以在实验中被直接测量和操控，为超辐射系统的控制提供了新的实验途径。
...
### [6] [Probing individual phonon-polaritonic nanoparticle-on-mirror cavities by infrared nanospectroscopy](https://arxiv.org/abs/2607.07941)
...
TL;DR: 本文研究了基于声子极化激元材料的纳米粒子-镜面（NPoM）腔在红外波段的近场响应。通过纳米FTIR光谱实验观测到两种腔模共振，数值模拟表明该腔具有超小模体积、高品质因子和极高场增强，且纳米FTIR探针可实现高效激发与读出，为红外纳米光子学和超灵敏振动光谱学提供了新平台。


<details>
  <summary>Details</summary>
Motivation: 探索声子极化激元材料在中红外波段实现NPoM腔的可能性，填补该领域的研究空白。

Method: 采用纳米FTIR光谱技术测量单个金纳米粒子在石英基底上的NPoM腔的近场响应，结合数值模拟分析腔模特性及探针影响。

Result: 观察到两种可重复的腔共振模式（基模和二阶天线模）；无探针时NPoM腔具有超小模体积（~10³ nm³）、高品质因子（~100）、极高场强增强（~10⁴）和Purcell因子（~10⁹）；纳米FTIR探针可高效激发和读出模而不扰动腔模，并将局域场增强两个数量级。

Conclusion: 声子极化激元NPoM腔是红外纳米光子学的有前景平台，为基于纳米FTIR的超灵敏振动光谱学测量铺平了道路。
...
### [7] [Ultra-high-speed chemiluminescence tomography of spinning-mode detonation waves](https://arxiv.org/abs/2607.07971)
*Amit K. Singh,Mateo Gomez,Kevin Y. Cho,Aaron W. Skiba,Samuel J. Grauer*

Main category: physics.optics

TL;DR: 本文提出了一种化学发光断层成像技术，用于重建乙烯基混合燃料在1大气压下传播的爆震波中时间分辨的三维反应结构。通过五台相机同步记录蓝宝石圆柱测试段内的化学发光图像，并开发了定制标定程序以校正圆柱折射效应。以兆赫兹速率重建有效发射源项场，展示了持续旋转爆震、失效旋转爆震及对向传播横向波三种案例。重建场可揭示波形形态、确定轴向和方位角波速、估算关键几何与运动学参数。结果表明，时间分辨化学发光断层扫描可作为非侵入式工具解析体积爆震动力学，弥补点、平面或视线诊断的不足。


<details>
  <summary>Details</summary>
Motivation: 爆震波中的三维反应结构难以通过传统的点、平面或视线诊断方法进行解析，需要一种非侵入式的高时空分辨率成像技术来捕捉其复杂动力学行为。

Method: 采用五台高速相机同时记录通过蓝宝石圆柱段爆震波的化学发光图像；开发定制标定程序校正圆柱体折射效应；将多视角图像融合以兆赫兹速率重建有效发射源项的三维场分布。

Result: 成功重建了三种爆震波形态的三维反应结构：持续旋转爆震、失效旋转爆震以及对向传播横向波案例。获取了波形形态可视化、轴向与方位角波速测量，以及关键几何与运动学参数估计。

Conclusion: 时间分辨化学发光断层成像是一种有效的非侵入式工具，能够解析体积爆震动力学，提供传统诊断手段难以获取的三维时空信息。
...
### [8] [Multiband topological group-velocity control from slow light to light stopping](https://arxiv.org/abs/2607.08055)
*Junhao Yang,Jiarui Wang,Jingyu Liu,Shirong Lin,Xinyuan Qi*

Main category: physics.optics

TL;DR: 本文在Harper-Hofstadter光子晶格中引入次近邻耦合，实现了拓扑慢光调控。次近邻耦合不仅能打开原本闭合的带隙，还能使边缘态色散平坦化，从而支持拓扑保护的慢光传输。通过调节耦合参数，可以实现多重拓扑光停泊态，为宽带拓扑慢光器件提供了新设计原理。


<details>
  <summary>Details</summary>
Motivation: 现有拓扑光子平台中边缘态群速度难以调控，且带隙数量有限，限制了拓扑慢光器件和宽带光子系统的应用。作者希望通过引入长程次近邻耦合来解决这一问题。

Method: 在Harper-Hofstadter光子晶格中引入次近邻耦合，通过理论计算带隙陈数，并通过传播模拟验证拓扑保护的慢光边缘态传输。

Result: 次近邻耦合打开了原本闭合的带隙，使边缘态色散平坦化；带隙支持符号相反的带隙陈数；模拟验证了反向手性边缘态的鲁棒拓扑慢光传输；三个拓扑带隙中均存在慢光边缘态，实现了宽带拓扑慢光；通过调节耦合参数可实现多重拓扑光停泊态。

Conclusion: 长程次近邻耦合是一种有效的拓扑群速度调控机制，为拓扑慢光器件、光学延迟线和集成多带光子系统的设计提供了新思路。
...
### [9] [Nyquist-Sampled Time-Domain Adjoint FDTD for Memory-Efficient Broadband Nanophotonic Inverse Design](https://arxiv.org/abs/2607.08159)
*Mingyu Park,Owen D. Miller,Haejun Chung*

Main category: physics.optics

TL;DR: 本文提出奈奎斯特采样伴随FDTD方法，通过仅存储奈奎斯特间隔的前向场历史，在保持双模拟缩放的同时将内存开销降低高达107倍，证明宽带时域伴随优化中的内存瓶颈源于冗余时序场存储而非梯度计算的内在需求。


<details>
  <summary>Details</summary>
Motivation: 传统时域伴随优化面临严重的内存瓶颈，需要保留每个FDTD时间步的前向场历史，这限制了大规模三维纳米光子学逆向设计的可行性。

Method: 利用奈奎斯特采样定理，仅在前向模拟中存储符合奈奎斯特间隔的时间步场，并在反向伴随传播中使用稀疏场历史进行梯度累积，实现在线梯度计算而不需保留完整的前向或伴随场历史。

Result: 在四个二维宽带纳米光子学基准和一个全三维超透镜上，该方法保持梯度保真度和优化器件性能，同时将主导场存储内存降低高达107倍。

Conclusion: 宽带时域伴随FDTD中的主要内存障碍不是梯度评估的固有要求，而是冗余时序场存储的结果，为大规模三维纳米光子学逆向设计开辟了实用路径。
...
### [10] [Nanosecond Pulsed-Laser Treatment Couples Chloride Removal with Oxide Transformation in Salt-Corroded Carbon Steel](https://arxiv.org/abs/2607.08229)
*Youichi Ishikawa,Yoshitaka Okuyama,Daishi Fujita*

Main category: physics.optics

TL;DR: 本研究采用约150 ns FWHM、拖尾延伸至约600 ns的Q开关脉冲光纤激光器处理盐污染SS400碳钢，在100 W（100 kHz）条件下实现了Na和Cl的近背景值去除，并将多孔锈层转化为致密的磁铁矿层，揭示了单步双阶段机制：高峰值功率前沿驱动烧蚀，而600 ns拖尾提供持续热能促进氧化物重凝和相变。


<details>
  <summary>Details</summary>
Motivation: 海洋环境中碳钢表面处理需要同时清除腐蚀产物和氯化物污染物并改性残余氧化层，现有方法难以在单一步骤中实现这些目标。

Method: 使用Q开关脉冲光纤激光器（FWHM约150 ns，拖尾约600 ns，脉冲能量1 mJ），在50-200 kHz重复频率范围内处理盐污染SS400碳钢，通过WDS-EPMA、SEM和XPS表征处理效果。

Result: 100 kHz条件下Na和Cl降至近背景值；多孔锈层转变为致密激光改性层；XPS证实Fe³⁺卫星特征被抑制，表明赤铁矿主导的锈层转变为保护性磁铁矿层。

Conclusion: 揭示单步双阶段机制：脉冲前沿高峰值功率驱动烧蚀，600 ns拖尾提供持续热能促进氧化物重凝和相变。该方法提供了一种有前景的非接触式单步净化与表面功能化方案。
...
### [11] [Ion-Implanted Silicon Nanoregions Enable Ultra-Low-Loss Trimming of Cladded Photonic Integrated Circuits](https://arxiv.org/abs/2607.08266)
...
TL;DR: 本文提出利用硅离子注入技术在包层光子集成电路（PIC）中实现可扩展、超低损耗的后制造修整，通过局部形成高折射率富硅区，在光子交叉阵列上将通道输出变异从78.5%降至5.9%，且额外损耗低于0.001 dB/π相移。


<details>
  <summary>Details</summary>
Motivation: 随着光子集成电路复杂度增加，制造引起的响应偏差不断累积，后制造修整对于释放其全部潜力至关重要，但现有方法在可扩展性和低损耗方面存在局限。

Method: 采用硅离子注入技术，在包层中局部形成高折射率富硅纳米区域，实现对波导相位的精确调控，同时避免对底层波导造成损伤。

Result: 注入引起的额外损耗低于0.001 dB/π相移；光学响应在四个月内保持稳定；在光子交叉阵列上实现自动化修整，平均通道输出变异从78.5%降低至5.9%。

Conclusion: 该工作为大规模包层光子集成电路提供了一条自动化、超低损耗的后制造修整实用路径。
...
### [12] [Inverse-designed meta processing units for multi-task near-field photonic computing](https://arxiv.org/abs/2607.08360)
*Chu Wu,Zeyu Cai,Songtao Yang,Ruoyu Shen,Yinan Zhao,Haiou Zhang,Wei Chu,Xing Lin*

Main category: physics.optics

TL;DR: 本文提出了一种基于逆向设计的元处理单元（MPU），可在浅刻蚀硅区域内实现紧凑的2x2复矩阵变换。MPU尺寸仅9.6μm×4.8μm，可作为可复用无源矩阵基元与可重构MZI神经元结合。实验验证了3比特量化的MZI等效酉矩阵库（有效精度3.32位）、任意复矩阵拟合、级联4x4矩阵操作（保真度92.7%），并在双任务元音识别中达到83.5%和80.9%的测试准确率。大规模EMNIST模拟中，细粒度神经元级MPU替换策略在90%共享率下达到87.64%平均准确率，比层级基线高出7.26个百分点。


<details>
  <summary>Details</summary>
Motivation: 集成光子神经网络需要同时具备紧凑性、矩阵通用性和任务级可重构性的光学算子，现有方案难以兼顾这三者。本文旨在通过逆向设计构建一种紧凑、通用且可重构的无源矩阵算子。

Method: 采用逆向设计方法在浅刻蚀硅区域中设计元处理单元（MPU），实现局域复矩阵变换。MPU作为可复用无源矩阵基元，与可重构MZI光子神经元结合，支持层内和层间异构集成。采用硬件在环训练策略进行系统级优化。

Result: 2x2 MPU尺寸9.6μm×4.8μm；3比特量化的酉矩阵库有效精度3.32位；级联4x4矩阵保真度92.7%；双任务元音识别准确率83.5%和80.9%；EMNIST模拟中细粒度替换策略在90%共享率下准确率87.64%，优于层级基线7.26个百分点。

Conclusion: 逆向设计的MPU可作为紧凑无源矩阵算子，为异构、硬件自适应的光子神经网络提供高效的可重构矩阵计算基元，兼顾面积效率、矩阵泛化能力和任务级可重构性。
...
### [13] [Joint Discrete-Continuous Flow Matching for Open-Vocabulary Inverse Design of Multilayer Optical Coatings](https://arxiv.org/abs/2607.08392)
*Zhiyi Li,Yuheng Jin,Yidan Huang,Nan Chen,Hongyan Fu,Yikun Bu*

Main category: physics.optics

TL;DR: IrisFlow是一个基于查询的开放词汇流匹配框架，用于多层光学镀膜设计。它支持在查询时动态指定目标光谱、波长网格、候选材料光学常数和层数，通过离散流匹配采样材料序列、连续流匹配生成厚度，无需重新训练即可泛化到未见材料，并在实验中成功制造了四种彩色冷光器件。


<details>
  <summary>Details</summary>
Motivation: 现有神经逆向设计方法属于封闭世界，组件选择受限于固定词汇表、坐标网格在训练时冻结、连续变量被离散化为序列token，缺乏灵活性和泛化能力。多层光学镀膜作为重要工业场景，需要同时处理材料序列、层厚和波长依赖响应，现有方法无法支持开放词汇的查询式设计。

Method: IrisFlow采用查询式开放词汇流匹配框架：(1) 候选材料以波长感知的光学token（而非学习到的身份标识）输入；(2) 材料序列通过离散流匹配在查询的候选库中采样；(3) 厚度通过连续流匹配生成（无需离散化）。模型参数1.36亿，可设计2-100层堆栈。

Result: 在224任务基准上忠实重建分布内目标；对15种未见材料保持同等精度无需重新训练；可重建超出训练范围1100 nm的光谱波段；在基线材料库上优于自回归基线。制造的四款彩色冷光器件中，三款色度器件CIEDE2000色差3.1-5.2，同时保持93-95%太阳近红外反射率。

Conclusion: IrisFlow证明了开放词汇神经逆向设计从仿真到实际制造的全链路可行性，通过查询式流匹配实现了对材料、厚度、波长网格的灵活泛化，为工业级光学镀膜设计提供了新范式。
...
### [14] [Doubly resonant enhancement of second-harmonic generation with in-plane phase matching in plasmonic metasurfaces on an AlInP slab waveguide](https://arxiv.org/abs/2607.08430)
*Timo Stolt,Huayu Bai,Seyed Ahmad Shahahmadi,Jani Oksanen,Andriy Shevchenko,Radoslaw Kolkowski*

Main category: physics.optics

TL;DR: 本文实验展示了利用高Q值导模共振的混合金属-介质超表面，通过面内相位匹配实现二次谐波产生两个数量级的增强，Q值在可见光和近红外波段分别超过400和900。


<details>
  <summary>Details</summary>
Motivation: 传统非线性晶体需要严格的相位匹配条件，而非线性超表面虽能规避此限制，但效率仍有待提升。作者希望通过高Q值导模共振来增强超表面的二次谐波产生效率。

Method: 采用金纳米盘周期性阵列与AlInP平板波导构成的混合超表面，利用导模共振实现高Q值谐振，在频率和动量分辨测量中实现双谐振条件下的二次谐波增强。

Result: 实验测得可见光波段Q值超过400，近红外波段Q值超过900；在双谐振条件下二次谐波产生实现了两个数量级的增强。

Conclusion: 面内相位匹配和空间模式重叠是增强的主要机制，该策略为高效超表面频率转换器及紧凑光子系统的应用提供了一条有前景的路径。
...
### [15] [Optimal slit width for high-precision orbital angular momentum measurement using angular double-slit interferometry](https://arxiv.org/abs/2607.08549)
*Yu Jian,Xin Wang,Jiyang Zhang,Tao Chen,Manpeng Chang,Chen Liu,Weimin Wang*

Main category: physics.optics

TL;DR: 本文提出并优化了角向双缝干涉方法，通过精确匹配缝宽与螺旋相位周期（2π/|l|），实现了涡旋光束轨道角动量（OAM）的高精度测量。实验验证了l=5,10,15时该准则的普适性，并通过引入附加相位偏移确定了拓扑荷符号。


<details>
  <summary>Details</summary>
Motivation: 现有涡旋光束OAM测量方法在精度和鲁棒性方面存在不足，需要一种简单、高精度且能同时确定拓扑荷大小和符号的测量方案。

Method: 采用动态双缝扫描，缝宽精确匹配螺旋相位周期2π/|l|以避免相位截断或周期重叠；通过扫描动态双缝，从轴上光强振荡频率直接确定拓扑荷大小；引入附加相位偏移确定拓扑荷符号。

Result: 建立了缝宽选取的关键准则：缝宽必须精确等于2π/|l|；在此最优条件下干涉条纹可见度最高、测量误差最小；实验验证了l=5,10,15的普适性；成功区分了l=10和l=-15的符号。
...
### [16] [Broadband silicon photonic phase shifters driven by gradient optical forces](https://arxiv.org/abs/2607.08558)
...
TL;DR: 本文演示了一种基于光机械力梯度的全光硅光子移相器，利用悬空槽模波导和亚波长光栅实现光-光直接控制，仅需60 μW泵浦功率即可在178.6 μm器件中实现π相位偏移，避免了电控和Kramers-Kronig吸收限制。


<details>
  <summary>Details</summary>
Motivation: 现有硅光子可编程系统中光信号仍需电子控制，光直接控制光的方案仍很有限，亟需开发全光控制的相位调制方案。

Method: 采用悬空槽模波导结合亚波长光栅提供机械支撑并保持光学约束，利用梯度光力产生的光机械介导的光-光相互作用，实现泵浦光束与信号光束共传播的全光相位调制。

Result: 在178.6 μm长器件中仅需60 μW泵浦功率即可实现π相位偏移；实验测量与集总力平衡模型定量吻合。

Conclusion: 该全光几何变形驱动的方案无需局部电连接、无Kramers-Kronig吸收损耗，且天然兼容低温量子光子平台，适用于线性光学处理器、神经形态光子网络、量子光子电路等应用。
...
### [17] [Intracellular luminescence thermometry: A story of disagreement, trust, and hope](https://arxiv.org/abs/2607.08574)
*Araceli de Aquino Samper,Liyan Ming,Daniel Jaque,Riccardo Marin*

Main category: physics.optics

TL;DR: 本文系统评述了细胞内发光测温法的现状，指出该领域在温度梯度的幅度和物理合理性上仍存在争议，强调了实验严谨性对测量意义的关键作用，并分析了偏差和交叉敏感性两大误差来源，最后提出提高可重复性的实践方向。


<details>
  <summary>Details</summary>
Motivation: 细胞内发光测温法有望揭示热量在活细胞内的产生、耗散和调节机制，但该领域对细胞内温度梯度的幅度和物理合理性存在重大争议，需要系统梳理和批判性审视。

Method: 本文采用综述性讨论方法，系统梳理了发光测温法的理论基础（热传导概念、发光测温原理及性能指标），深入分析了影响热读出的关键因素（测温设计、细胞内定位、校准、显微配置和数据处理），并重点关注偏差（测量条件和光学畸变）和交叉敏感性（pH、粘度、离子强度、生物分子相互作用等非温度参数的干扰）两大误差源。

Result: 明确指出细胞内温度梯度的测量结果仍存在争议，偏差和交叉敏感性是影响测量可靠性的两大反复出现的误差来源。

Conclusion: 为提高可重复性和可靠性，本文提出多特征读数、机器学习辅助分析和FAIR数据实践等实用方向，并为未来研究提供了建议。
...
### [18] [Asymmetric high-harmonic generation from subwavelength bianisotropic resonators](https://arxiv.org/abs/2607.08715)
*Albert Mathew,Piyush Jangid,Rebecca Aschwanden,Yves Koppeler,Thomas Zentgraf,Sergey Kruk*

Main category: physics.optics

TL;DR: 本文实验证明单个介电亚波长谐振器可作为方向选择性的高次谐波源，通过米氏模式的激发与杂化，在泵浦波长下0.12λ³的几何体积和0.03λ³的光学模式体积内，实现对三、五、七次谐波的正反向不对称生成。结构不对称性沿传播方向转化为不同照明方向下的模式耦合差异，建立了双各向异性亚波长谐振器作为柔性非对称高次谐波生成的平台。


<details>
  <summary>Details</summary>
Motivation: 高次谐波产生（HHG）虽已广泛关联于气体和等离子体，但纳米结构固体作为新兴替代源，能够增强并控制HHG。本研究旨在探索单个介电亚波长谐振器能否作为方向选择性的高次谐波源，并通过米氏模式实现谐波阶次的控制。

Method: 实验上，采用单个介电亚波长谐振器，利用其结构沿传播方向的不对称性，在不同照明方向下激发不同的模式耦合；理论上，通过米氏模式的激发与杂化描述高次谐波生成过程，分析谐振器几何体积与光学模式体积对谐波产生的影响。

Result: 谐振器的几何体积为0.12λ³，光学模式体积为0.03λ³（泵浦波长下）。结构不对称性导致不同照明方向下的模式耦合差异，从而在三、五、七次谐波中产生显著的前后方向不对称性。

Conclusion: 双各向异性亚波长谐振器可作为柔性非对称高次谐波生成的平台，扩展了利用米氏共振纳米光子学控制强场光-物质相互作用的工具库。
...
### [19] [A Puck-informed mode-resolved phase-field fatigue framework for unidirectional composites](https://arxiv.org/abs/2607.07977)
*Aamir Dean*

Main category: physics.comp-ph

TL;DR: 本文提出了一种基于Puck理论的模式分辨相场疲劳框架，将单向纤维增强复合材料的疲劳断裂分为纤维主导和基体/纤维间两个独立通道，每个通道具有独立的疲劳历史、阈值和抗力退化规律。通过在Abaqus/Standard中实现，该框架无需取向或几何特定调参即可复现0°、45°和90°方向的多种疲劳断裂模式，疲劳寿命排序符合预期。


<details>
  <summary>Details</summary>
Motivation: 单向纤维增强复合材料的疲劳断裂具有强模式依赖性（横向/偏轴循环由基体和纤维间机制主导，纤维对齐循环则激活纵向通道），而现有的单损伤变量模型虽能拟合整体刚度损失，却无法识别活跃的断裂机制，亟需一种能够区分不同疲劳通道的框架。

Method: 基于Puck理论构建模式分辨的相场疲劳框架：设置纤维主导和基体/纤维间两个独立疲劳通道，各通道有其疲劳历史、阈值和抗力退化律；疲劳不直接退化弹性刚度，而是降低活跃通道的断裂抗力，由相场控制刚度损失和裂纹路径演化；在Abaqus/Standard中采用紧凑的UMAT-UEL架构实现。

Result: 使用固定IM7/8552材料和疲劳参数，通过单单元测试、参数扫描以及中心缺口和开孔拉伸案例（0°、45°、90°）在单调和循环加载下验证。无需调参即可复现90°横向基体/纤维间开裂、45°偏轴开裂、0°纵向基体分裂及延迟纤维激活；疲劳寿命排序正确（45°和90°约1000次循环失效，0°达200000次无纤维开裂）。

Conclusion: 该工作为数值验证和跨几何一致性评估，不声称标定实验寿命预测；额外载荷、孔径、网格、长度尺度和循环块研究确认了裂纹模式一致性和收敛趋势。
...
### [20] [Parallel simulation of rarefied gas flows on unstructured meshes using the DIG-augmented DSMC method](https://arxiv.org/abs/2607.08195)
*Tao Huang,Liyan Luo,Hong Deng,Lei Wu*

Main category: physics.comp-ph

TL;DR: 本文针对稀薄气体流动模拟中DSMC方法在近连续流域计算成本过高的问题，开发了一种基于DIG加速的并行DSMC求解器。该求解器在非结构网格上实现了DSMC与宏观合成方程的间歇耦合，采用混合MPI架构和动态负载平衡策略，显著降低了计算内存和运行时间，数值结果与SPARTA DSMC吻合良好。


<details>
  <summary>Details</summary>
Motivation: DSMC方法在近连续流域计算成本极高，而DIG方法虽能加速耦合DSMC与宏观合成方程，但现有研究缺乏针对三维非结构网格的并行实现，难以处理复杂几何外形的高保真稀薄气体流动模拟。

Method: 在标准DSMC粒子输运和碰撞算法基础上，构建间歇性耦合框架实现随机DSMC模块与确定性宏观合成方程之间的宏观流场数据交换；采用带虚拟细胞的混合MPI架构实现局部粒子追踪和跨进程粒子迁移；集成基于图划分的动态负载平衡策略以缓解计算域内粒子分布不均问题。

Result: 所提求解器与SPARTA DSMC数值结果吻合良好；利用DIG方法的快速收敛和渐进保持特性，所需空间网格数和统计采样步数大幅减少，计算内存和运行时间显著降低。

Conclusion: 本文为复杂几何构型上的稀薄气体高保真模拟提供了一种高效的高性能数值工具，代码已在GitHub开发者仓库开源。
...
### [21] [Learned Committors as Reaction Coordinates for Nucleation Rates](https://arxiv.org/abs/2607.08207)
*Hubert J. Naguszewski,David Quigley*

Main category: physics.comp-ph

TL;DR: 本文利用卷积神经网络（$p_B$-NN）作为committor的代理，训练于暴力计算得到的committor标签，并将其直接用作马尔可夫状态模型的坐标。应用于二维Ising模型的磁化反转，$p_B$-NN在多个热力学条件下均能复现暴力计算的成核速率。研究发现，有效的反应坐标只需可靠地分离亚稳态和稳态盆地，无需在点级别精确预测每个微观状态的committor。这一发现对成核稀有事件模拟中集体变量的选择具有直接启示。


<details>
  <summary>Details</summary>
Motivation: 一阶相变分析中的核心挑战是识别最优反应坐标。理论上committor是理想选择，但其计算成本过高，传统上难以实现。本文旨在通过神经网络代理committor来解决这一可计算性问题。

Method: 训练卷积神经网络（$p_B$-NN）作为committor的代理模型，利用暴力模拟生成的committor标签进行监督学习，然后将训练好的网络输出直接作为马尔可夫状态模型的反应坐标。

Result: 在二维Ising模型磁化反转任务中，$p_B$-NN在不同热力学条件下均能准确复现暴力计算的成核速率。此外，最大几何簇大小虽在点wise级别对committor的预测不佳，但仍能给出准确的成核速率。

Conclusion: 有效的反应坐标不必在每个微观状态上精确拟合committor，只需可靠地区分亚稳态和稳态盆地即可。这一结论对成核稀有事件模拟中集体变量的选择具有重要指导意义。
...
### [22] [Repurposing acquisition devices into trigger-based timing synchronization of breakdown events during MITICA high voltage holding experiments](https://arxiv.org/abs/2607.08501)
...
TL;DR: 本文针对MITICA（ITER加热中性束注入器全尺寸样机）的1MV真空绝缘验证需求，提出了一套基于低成本嵌入式RedPitaya（Zynq-7000 FPGA）器件的时间重建架构。该系统作为可配置触发多路复用器和瞬态记录仪，通过自校准光纤和内部门控延迟，实现了稀疏连接图拓扑下多采集设备的波形同步。


<details>
  <summary>Details</summary>
Motivation: MITICA装置需承受1MV连续高压，真空击穿事件（电压骤降、强电流放电）的精确时序重建对理解击穿机理和验证绝缘性能至关重要，现有系统难以在低成本下实现稀疏分布式设备的纳秒级同步。

Method: 采用RedPitaya（Zynq-7000 FPGA）作为时序节点，兼具触发多路复用和瞬态波形记录功能；通过自校准技术测量光纤静态固有延迟和内部逻辑延迟，生成偏移量以对齐稀疏连接图拓扑中各采集设备与中枢节点的波形数据。

Result: 实现了基于嵌入式FPGA的低成本时间同步架构，能够离线重建真空击穿事件的相对时序序列，并通过自校准补偿光纤和逻辑延迟。

Conclusion: 该架构为MITICA的1MV真空击穿事件分析提供了经济高效的时序同步方案，有望推广至其他需要分布式高速采集同步的脉冲功率或等离子体诊断场景。
...
### [23] [TeV Electron Beams from Plasma Acceleration via Regenerative Cascading](https://arxiv.org/abs/2607.07979)
*Chaojie Zhang,Chan Joshi*

Main category: physics.acc-ph

TL;DR: 本文提出一种基于再生级联的等离子体尾场加速方案，其中每一级自注入新鲜尾随电子束，加速后的尾随束成为下一级的驱动束，实现能量倍增而非相加。两阶段亚公里级等离子体加速器PIC模拟显示，由45 GeV、100 nC驱动束可产生1.1 TeV、0.12 nC、能散~0.3%的电子束，低能散通过动态束负载在耗尽后驱动器的演化尾场中实现。


<details>
  <summary>Details</summary>
Motivation: 传统等离子体加速器虽能产生比常规射频加速器高数个量级的加速梯度，但大多数通向TeV能量的方案仍需数十个级联阶段，每阶段都要求亚微米对准、飞秒同步以及精确匹配加速尾随束，实现难度极大。

Method: 引入再生级联等离子体尾场加速（regenerative cascading），每级自注入新鲜尾随电子束，加速后的尾随束变为下一级的驱动束；利用动态束负载（dynamic beam loading）在耗尽后驱动器的演化尾场中作为内置能量去啁啾器，实现低能散。

Result: 两阶段亚公里级等离子体加速器PIC模拟产生1.1 TeV电子束，均方根能散~0.3%，电荷量0.12 nC，由45 GeV、100 nC驱动束驱动。

Conclusion: 再生级联方案具有能量倍增而非相加、自动对准同步匹配、每级尾随束亮度重置等优势，为通向TeV能量级等离子体加速器提供了可行路径。
...
### [24] [Multi-GeV Electron Combs from a Plasma Wakefield Accelerator](https://arxiv.org/abs/2607.08069)
...
TL;DR: 本文报道了在等离子体加速器中通过原位注入生成多GeV电子梳（10个以上微束团）的技术，实现了能量和时间维度同时分离的飞秒级相空间整形，微束团具有百分级能散、高达10%的能量间隔和皮库级电荷量。


<details>
  <summary>Details</summary>
Motivation: 利用等离子体加速器独特的原位注入能力，在飞秒或更短时间尺度上对电子束相空间进行整形，以产生具有定制能量-时间结构的电子束。

Method: 驱动束在其自激发的等离子体尾场中周期性收缩，通过电离嵌入的氦原子在连续的自由振荡处顺序注入微束团；同时利用缓慢变化的等离子体密度将每个微束团映射到不同的尾场相位，将在~17 cm区域捕获的电子压缩成仅微米长的电子梳。

Result: 成功生成了包含10个以上微束团的多GeV电子梳；单个微束团具有百分级能散、高达10%的能量间隔和数皮库电荷；百分级能散和抛物线能量间隔趋势提供了亚飞秒微束团持续时间和飞秒级间隔的实验证据。

Conclusion: 该工作展示了等离子体加速器中飞秒级原位相空间整形能力，为产生具有定制能量-时间结构的电子束开辟了道路，推动等离子体加速器向实用化自由电子激光等应用迈进。
...
### [25] [Analysis of collisional and facility effects in a magnetic nozzle plasma expansion](https://arxiv.org/abs/2607.07861)
*Manuel Cortés-Hernán,Mario Merino,Diego García-Lahuerta,Eduardo Ahedo*

Main category: physics.plasm-ph

TL;DR: 本文提出了一个轴对称、准中性的三流体模型，研究磁喷嘴中在存在中性粒子（来自等离子体源或均匀背景）情况下的等离子体膨胀。与其它模型不同，电子羽流冷却通过将对流作为主要电子能量通量实现，无需假设任何反常电阻。在电子高磁化极限下，电子主要参量可沿磁力线积分。电离、弹性及电荷交换碰撞不会改变无碰撞模型的主要定性特征。电离增强了喷嘴中的等离子体流，并导致额外的电子冷却，从而降低沿喷嘴的电势降。喷嘴效率通过磁推力增益和羽流发散角量化。讨论了两种电子流边界条件：喷嘴喉部的局域电流偶极条件和外部边界（即金属真空室壁）的全局无电流条件。后者在物理上更可靠：它通过塑造偶极电场引入腔壁对等离子体膨胀的影响；允许外推至未扰动的自由空间条件；并且能更好地近似随背景压力变化的实验趋势。


<details>
  <summary>Details</summary>
Motivation: 现有的磁喷嘴等离子体膨胀模型在考虑中性粒子影响时，往往需要引入反常电阻等假设来解释电子冷却机制，缺乏物理自洽性。本文旨在建立一个无需反常电阻假设的三流体模型，准确描述中性粒子的碰撞效应，并探讨不同边界条件对等离子体膨胀和喷嘴效率的影响。

Method: 建立了轴对称、准中性的三流体模型（电子、离子、中性粒子），在电子高磁化极限下沿磁力线积分电子主要参量。将电子能量通量处理为对流主导，从而在不引入反常电阻的情况下实现电子羽流冷却。模型中考虑了电离、弹性碰撞和电荷交换碰撞。对比了两种边界条件：喉部局域电流偶极条件和外边界全局无电流（金属壁）条件。

Result: 1) 中性粒子的碰撞（电离、弹性、电荷交换）不改变无碰撞模型的主要定性特征；2) 电离增强等离子体流并导致额外电子冷却，降低喷嘴电势降；3) 全局无电流边界条件在物理上更可靠，能通过塑造偶极电场引入腔壁影响，可外推至自由空间条件，且更好地拟合随背景压力变化的实验趋势。

Conclusion: 本文提出的三流体模型能够自洽地描述磁喷嘴中考虑中性粒子碰撞的等离子体膨胀过程，无需反常电阻假设。全局无电流边界条件是更可靠的物理边界条件，对于准确预测喷嘴性能和解释实验现象至关重要。
...
### [26] [Sensitivity of a low-shear heliotron configuration to localised ferrite perturbations](https://arxiv.org/abs/2607.08290)
...
TL;DR: 本文研究了Heliotron J装置中铁素体钢对低剪切仿星器/螺旋器磁位形的影响。通过点偶极磁化模型数值评估真空室内部不同位置铁素体钢板的影响，发现旋转变换和磁岛宽度对安装位置敏感，这种敏感性源于背景非轴对称场与铁素体扰动的耦合。外侧直线段安装产生最显著的磁拓扑变化，并对M=4环向周期性的破坏最敏感。此外，合理布置被动磁偶极子可在保持真空磁阱深度的同时降低有效螺旋波纹，并在低场实验中可诱导仿星器非对称边界扰动。


<details>
  <summary>Details</summary>
Motivation: 探究铁素体钢材料在仿星器/螺旋器装置中的磁效应，特别是其对磁位形的影响机制和敏感度，以及利用被动磁偶极子优化磁位形的可能性。

Method: 采用点偶极磁化模型，在Heliotron J装置真空室内部多个位置数值评估铁素体钢板的影响，分析旋转变换、磁岛宽度、有效螺旋波纹和真空磁阱深度等关键磁位形参数的变化。

Result: 铁素体钢板安装位置对磁位形影响显著，外侧直线段安装产生的磁拓扑变化最大且对M=4环向周期性破坏最敏感；适当布置被动磁偶极子可在保持真空磁阱深度同时降低有效螺旋波纹。

Conclusion: 铁素体钢对低剪切仿星器磁位形的影响高度依赖于安装位置，通过合理设计被动磁偶极子布局可实现磁位形优化，为未来仿星器装置的材料选择和磁位形控制提供理论依据。
...
### [27] [Precision mapping of laser-driven magnetic fields and their evolution in high-energy-density plasmas](https://arxiv.org/abs/2607.08680)
*Lan Gao,PM Nilson,IV Igumenshchev,MG Haines,DH Froula,R Betti,DD Meyerhofer*

Main category: physics.plasm-ph

TL;DR: 利用超快质子射线照相技术，在激光加速平面箔靶中测量了由瑞利-泰勒不稳定性产生的MG量级磁场，实验数据与2-D磁流体动力学模拟结果吻合良好。


<details>
  <summary>Details</summary>
Motivation: 研究激光加速靶中瑞利-泰勒不稳定性产生的自生磁场，通过实验测量验证理论预测，以理解高能量密度物理中的磁场演化机制。

Method: 在OMEGA EP激光系统上，使用~4 kJ、2.5 ns激光脉冲照射薄塑料箔靶（强度~10¹⁴ W/cm²），利用超快质子射线照相术测量靶加速过程中瑞利-泰勒不稳定性产生的磁场。

Result: 实验观测到了靶的水动力学演化以及箔靶破裂后产生的MG级（百万高斯级）磁场，实验结果与2-D磁流体动力学模拟预测高度一致。

Conclusion: 实验证明了瑞利-泰勒不稳定性可在激光加速靶中产生强磁场，且2-D MHD模拟能够准确预测此类复杂自生磁场现象。
...
### [28] [Physics-informed neural networks for shock capturing in inviscid flows around an airfoil](https://arxiv.org/abs/2607.08130)
*Jiahao Song,Wenbo Cao,Weiwei Zhang*

Main category: physics.flu-dyn

TL;DR: 本文指出PINNs在求解含激波的稳态无粘绕流问题时失败的原因是稳态欧拉方程约束过弱，导致梯度下降收敛到光滑局部最优。为此，作者提出将稳态激波捕捉重构为时间演化过程，引入非稳态欧拉方程作为强约束，并设计了嵌入伪时间步进的损失函数以及简化的欧拉方程形式，在四个前向问题中验证了有效性。


<details>
  <summary>Details</summary>
Motivation: PINNs在求解含激波的流体力学问题（尤其是翼型绕流的稳态无粘流动）时存在失效或无法捕捉激波的问题，需要从根本上分析原因并提出改进方法。

Method: (1) 将稳态激波捕捉重构为时间演化过程，用引入时间导数项的非稳态欧拉方程替代稳态方程作为PINNs的物理约束；(2) 引入嵌入伪时间步进概念的PDE损失函数以解决长时间维度求解困难；(3) 开发了欧拉方程的简化形式以提高激波捕捉精度。

Result: 在四种不同流动条件和几何形状的前向问题中，所提方法成功捕捉激波结构，验证了方法的有效性。

Conclusion: 通过将稳态问题转化为时间演化问题并引入伪时间步进策略，可以有效克服PINNs在含激波稳态流动中的光滑局部最优收敛问题，提升激波捕捉能力。
...
### [29] [Trapping-loss transition via a saddle-node bifurcation in thermophoretic particle transport driven by a time-periodic vortex](https://arxiv.org/abs/2607.08223)
*Srikumar Warrier*

Main category: physics.flu-dyn

TL;DR: 本文研究了时间周期涡旋流中惯性粒子在惯性漂移与热泳迁移竞争下的动力学行为，建立了周期平均的径向迁移模型，发现粒子俘获通过鞍结分岔消失，并受无量纲参数Π控制。


<details>
  <summary>Details</summary>
Motivation: 理解非定常流中惯性粒子在多种迁移机制（惯性漂移与热泳）竞争下的动力学行为，揭示粒子俘获的物理机制及其丧失条件。

Method: 从小Stokes数粒子运动方程出发，推导出周期平均的径向迁移模型；通过相图、分岔图和局部渐近分析研究动力学行为；改变速度场和温度场验证结论的鲁棒性。

Result: 粒子俘获通过鞍结分岔消失；存在临界Π值，低于该值时稳定与不稳定不动点合并消失；伴随主导特征值消失和弛豫时间发散（临界慢化）。

Conclusion: 粒子俘获的丧失由鞍结分岔主导，该机制对不同速度场和温度场具有鲁棒性；无量纲参数Π是控制俘获行为的关键量。
...
### [30] [Tracking the boundary between absolute/convective instability using adjoint equations](https://arxiv.org/abs/2607.08305)
*Yue Xiao,Hui Li,Zijing Ding*

Main category: physics.flu-dyn

TL;DR: 本文提出了一种基于伴随增强伪弧长延续的直接方法，用于追踪频率仿射广义特征值问题的中性 stationary-saddle 边界，替代了传统的嵌套鞍点搜索和参数平面重建，显著降低了计算成本并提高了精度。


<details>
  <summary>Details</summary>
Motivation: 传统确定绝对/对流不稳定性边界的方法需要反复在复波数平面进行鞍点搜索，并在物理参数空间扫描以定位零绝对增长率。这种嵌套计算对于大型非正规特征值问题成本高昂，且对模态分支关联敏感。

Method: 将零群速度条件表达为伴随可解性残差，与直接和伴随特征问题、复规范约束及中性增长条件联合求解；在组合状态-参数空间中形成一维解流形，通过缩放伪弧长延续进行追踪，可跨越参数折叠而不切换延续变量。

Result: 该方法恢复了Ginzburg-Landau边界的解析形式；对于Gaussian-wake Orr-Sommerfeld问题，相对临界雷诺数的精度达约10^{-8}；计算速度比传统嵌套鞍点扫描快14.0-30.6倍；在Oldroyd-B自由表面薄膜问题中发现了中性鞍点流形的真正折叠和回滞边界几何。

Conclusion: 伴随增强伪弧长延续可直接高效地追踪中性边界本身，替代传统的嵌套鞍点搜索和参数平面重建，为不稳定性边界计算提供了一种计算高效且精确的新范式。
...
### [31] [On the rectification of oscillatory flows by flexible leaflets in a confined geometry](https://arxiv.org/abs/2607.08394)
*Omar Abukabsha,Simon Gsell,Martin Brandenbourger*

Main category: physics.flu-dyn

TL;DR: 本文通过数值模拟研究了低雷诺数下通道内多个非对称小叶片的集体相互作用，利用对称振荡板驱动流体，建立了稳态和时变条件下的分析框架，发现高叶片密度和最优弹黏数可最大化净输运流量。


<details>
  <summary>Details</summary>
Motivation: 生物系统中流体-结构相互作用可实现定向流体输运，但现有研究多局限于稳态表征，难以建模时变流场与多个相互作用结构之间的动态耦合，亟需发展能描述集体动力学行为的理论框架。

Method: 采用数值模拟方法，在低雷诺数条件下研究多个非对称叶片在通道内的集体相互作用；利用对称振荡板代替压差驱动以隔离背景不对称性；建立稳态极限下的解析输运评估框架，并扩展至时变通道振荡情形，提供完整的流固耦合动态描述。

Result: 高叶片密度可最大化集体相互作用和净输运量；定义了表征黏性水动力与叶片弹性恢复力之比的弹黏数，发现了使净流量最大的最优弹黏数值。

Conclusion: 该框架为分析黏性环境中集体细长结构的动态相互作用奠定了基础，为生物流体输运中的流动控制和微流控设计提供了理论支撑。
...
### [32] [Hele-Shaw Flow With Pressure and Shear Rate Dependent Viscosity](https://arxiv.org/abs/2607.08509)
*Benedetta Calusi,Liviu Iulian Palade*

Main category: physics.flu-dyn

TL;DR: 本文研究了一种粘度同时依赖于压力和剪切速率的流体在带有尖角几何结构的Hele-Shaw细胞中的行为，通过理论推导和数值模拟分析了尖角附近的流动特性。


<details>
  <summary>Details</summary>
Motivation: 实际应用需求驱动，包括生物医学设备设计、薄膜技术和注塑成型等领域，需要理解复杂流体在窄间隙几何中围绕尖角的关键流动特征。

Method: 基于不可压缩广义牛顿流体的控制方程，采用适当的流变模型，推导适用于Hele-Shaw流动的修正方程，获得尖角区域附近的特解，并通过数值模拟进行补充验证。

Result: 获得了尖角附近的特解，数值模拟结果展示了流变参数对流动行为的影响。

Conclusion: 该研究将先前关于纯压依赖性（压粘性）和屈服应力流体的分析拓展到粘度同时依赖于压力和剪切速率的流体，为受限复杂流动提供了新视角。
...
### [33] [Manifold-adapted radial basis functions for reduced-order modelling of chaotic flows](https://arxiv.org/abs/2607.08571)
*Miguel Pérez Cuadrado,Giorgio Maria Cavallazzi,Alfredo Pinelli*

Main category: physics.flu-dyn

TL;DR: 提出一种非侵入式降阶模型，通过聚类读取混沌系统的局部吸引子几何结构，利用自适应径向基函数库拟合降阶速度场，再结合运动学修正器稳定积分，无需控制方程即可复现长期统计特性（不变测度）。


<details>
  <summary>Details</summary>
Motivation: 混沌系统常演化在低维吸引子上，其几何结构随区域变化，传统全局方法难以捕捉这种局部差异性，因此需要一种能够自适应地读取局部几何的非侵入式建模方法。

Method: 首先通过聚类识别吸引子的局部区域，在每个区域构造自适应径向基函数库；然后通过一次全局正则化最小二乘求解将降阶速度场拟合到该库上，得到显式可微向量场；最后引入运动学修正器（其大小反映对学习场的依赖程度）来稳定积分，避免因径向基场远离数据衰减导致的逃逸问题。

Result: 在Lorenz-63上恢复吸引子、边际密度及正/中性Lyapunov指数，但强横向收缩恢复不足；在Lorenz-96上有效预测时间与调优的神经网络和储层计算相当，且能复现全状态和降阶可观测量的不变测度；在Kuramoto-Sivashinsky方程和准周期Kolmogorov流上匹配量化局部Galerkin模型的能量分布和谱，优于同维全局Galerkin投影。

Conclusion: 所提模型无需控制方程投影即可有效捕获混沌系统长期统计特性，为复杂动力系统的数据驱动降阶建模提供了一种实用且可解释的替代方案。
...
### [34] [Inverse Transfer and Coherence in Rotating Stratified Flow with Clouds and Phase Transitions](https://arxiv.org/abs/2607.08673)
*Yeyu Zhang,Yingshuo Peng,Leslie M. Smith*

Main category: physics.flu-dyn

TL;DR: 本文研究了含相变（水汽和液态水）的地球物理湍流中逆能量传递现象，发现与干动力学相比，湿过程会降低能量传递速率，并产生与非线性波相关的相干上升/下沉气流和大尺度模糊相边界。


<details>
  <summary>Details</summary>
Motivation: 过去四十多年，旋转和层结干动力学中的逆能量传递已被广泛研究，但真实大气包含水汽和相变过程，其对逆能量传递的影响尚不清楚。

Method: 采用Boussinesq动力学核心模型，加入水汽和液态水，采用渐进快云微物理极限，通过随机强迫进行数值计算，分析罗斯贝数和两个弗劳德数（干/饱和）表征的湿动力学逆能量传递。

Result: 湿动力学中能量传递速率较干动力学降低，原因是饱和区域具有更大的弗劳德数；最低阶非线性波影响湿位势涡度的逆尺度传递，这些波导致与模糊大尺度相边界对齐的相干上升/下沉气流。

Conclusion: 相变显著改变了地球物理湍流的逆能量传递特性，湿过程通过改变有效弗劳德数和引入相边界非线性波，抑制了能量逆传并形成了新的相干结构。
...
### [35] [Drift of interfaces in forced stably-stratified turbulence and the role of vertically-sheared helical structures](https://arxiv.org/abs/2607.08678)
*Niccolò Cocciaglia,Luca Biferale,Fabio Bonaccorso,Alessandra S. Lanotte*

Main category: physics.flu-dyn

TL;DR: 通过直接数值模拟研究发现，稳定分层湍流中密度界面的粗化动力学与螺旋结构的出现密切相关：螺旋结构导致界面漂移合并，而螺旋结构缺失则导致界面衰减。


<details>
  <summary>Details</summary>
Motivation: 实验观察到了稳定分层湍流中阶梯状密度剖面（由混合层和尖锐界面交替构成）的缓慢粗化现象，但对其中的动力学机制——界面合并与衰减的物理原因——尚不清楚。

Method: 采用直接数值模拟（DNS）对强制稳定分层湍流进行中等分辨率、极长时间积分的研究，分析螺旋度耗散率与浮力通量之间的动力学对应关系。

Result: 1）界面的垂直漂移和合并与空间均匀、垂直剪切的螺旋结构（破坏镜像对称性）的出现相关；2）螺旋结构缺失时，观察到界面衰减；3）建立了螺旋度浮力耗散率与垂直浮力通量之间的动力学对应关系。

Conclusion: 手性螺旋结构与界面的垂直运动之间存在因果联系，螺旋结构驱动了界面的漂移合并过程。
